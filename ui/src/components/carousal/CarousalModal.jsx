...
export const CarouselModal = ({ isOpen, onClose }) => {
  ...
  const validateInputs = () => {
    const validCafeImages = cafeImages.filter(url => url.trim());
    const validHomeScreenImages = homeScreenImages.filter(url => url.trim());

    if (validCafeImages.length === 0 && validHomeScreenImages.length === 0) {
      toast.error('Please add at least one image');
      return false;
    }

    const hasEmptyCafeFields = cafeImages.some(url => !url.trim());
    const hasEmptyHomeScreenFields = homeScreenImages.some(url => !url.trim());

    if (hasEmptyCafeFields || hasEmptyHomeScreenFields) {
      toast.error('Please fill all image fields or remove empty ones');
      return false;
    }
