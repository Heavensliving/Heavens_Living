...
import { FaEdit, FaSearch, FaFilter, FaFilePdf, FaArrowLeft, FaTrash } from 'react-icons/fa';
    ...
    <div className="container mx-auto p-2 md:p-6 bg-gray-100 rounded-lg shadow-lg">
      ...
      <div className={`${showFilters ? 'block' : 'hidden'} md:block mb-4`}>
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          {/* Category Dropdown */}
          <div>
            <label className="block text-sm md:text-base mb-1">Filter by Category:</label>
            <select
          ...
          <div>
            <label className="block text-sm md:text-base mb-1">Filter by Type:</label>
            <select
              value={selectedType}
          ...
          {/* Month Dropdown */}
          <div>
            <label className="block text-sm md:text-base mb-1">Filter by Month:</label>
            <select
              value={selectedMonth}
          ...
          {/* Year Dropdown */}
          <div>
            <label className="block text-sm md:text-base mb-1">Filter by Year:</label>
            <select
              value={selectedYear}
      ...
      <div className="flex flex-col sm:flex-row justify-between mb-4 gap-2">
        <div className="text-sm sm:text-base font-semibold bg-white p-2 rounded text-center">
          Total Records: <span className="text-gray-700">{sortedExpenses.length}</span>
        </div>
        <div className="text-sm sm:text-base font-semibold bg-white p-2 rounded text-center">
      ...
      <div className="overflow-x-auto bg-white rounded-lg shadow">
        <table className="min-w-full divide-y divide-gray-200">
          <thead className="bg-gray-300 text-black">
            <tr>
              <th className="py-3 px-2 sm:px-4 text-left text-xs sm:text-sm font-medium uppercase tracking-wider text-center">#</th>
              <th className="py-3 px-2 sm:px-4 text-left text-xs sm:text-sm font-medium uppercase tracking- text-center">Title</th>
              <th className="py-3 px-2 sm:px-4 text-left text-xs sm:text-sm font-medium uppercase tracking-wider text-center">Type</th>
              <th className="py-3 px-2 sm:px-4 text-left text-xs sm:text-sm font-medium uppercase tracking-wider text-center">Category</th>
              <th className="py-3 px-2 sm:px-4 text-left text-xs sm:text-sm font-medium uppercase tracking-wider text-center">Amt.</th>
              <th className="py-3 px-2 sm:px-4 text-left text-xs sm:text-sm font-medium uppercase tracking-wider text-center">Date</th>
              <th className="py-3 px-2 sm:px-4 text-left text-xs sm:text-sm font-medium uppercase tracking-wider text-center">Property</th>
              <th className="py-3 px-2 sm:px-4 text-left text-xs sm:text-sm font-medium uppercase tracking-wider text-center">Txn ID</th>
              <th className="py-3 px-2 sm:px-4 text-left text-xs sm:text-sm font-medium uppercase tracking-wider text-center">Bill</th>
              {admin.role === "Main-Admin" && (
                <th className="py-3 px-2 sm:px-4 text-left text-xs sm:text-sm font-medium uppercase tracking-wider text-center">Act.</th>
              )}
            </tr>
          </thead>
          <tbody className="bg-white divide-y divide-gray-200 text-center">
            {sortedExpenses.length > 0 ? (
              sortedExpenses.map((expense, index) => {
                ...
                return (
                  ...
                  >
                    ...
                    <td className="py-2 px-2 sm:px-4 whitespace-nowrap text-xs sm:text-sm">{expense.type}</td>
                    <td className="py-2 px-2 sm:px-4 whitespace-nowrap text-xs sm:text-sm">
                      {expense.category
                        .match(/.{1,10}/g) // Splits into chunks of 10 characters
                        .map((chunk, index) => (
                          <span key={index} className="block">{chunk}</span> // Displays each chunk on a new line
                        ))}
                    </td>
                    <td className="py-2 px-2 sm:px-4 whitespace-nowrap text-xs sm:text-sm">{expense.amount}</td>
                    <td className="py-2 px-2 sm:px-4 whitespace-nowrap text-xs sm:text-sm">
                    ...
                    <td className="py-2 px-2 sm:px-4 whitespace-nowrap text-xs sm:text-sm flex flex-col">
                      ...
                      <span>
                        {expense.paymentMethod === "Petty Cash" && expense.pettyCashType === "Cash"
                          ? "-"
                    ...
                    <td className={`py-2 px-2 sm:px-4 whitespace-nowrap text-xs sm:text-sm ${expense.billImg ? 'cursor-pointer' : ''}`}>
                      {expense.billImg ? (
                        <img
                    ...
                    {admin.role === "Main-Admin" && (
                      <td className="py-2 px-2 sm:px-4 whitespace-nowrap text-xs sm:text-sm flex justify-center space-x-3">
                        ...
                        <FaEdit
                          ...
                          onClick={() => navigate(`/editExpense/${expense._id}`)}
                        />

                        {/* Delete Button */}
                        <FaTrash
                          className="text-red-600 hover:text-red-800 cursor-pointer"
                          onClick={() => handleDelete(expense._id, expense.title, expense.amount, expense.billImg)}
                        />
                      </td>
                    )}
                  </tr>
              ...
              <tr>
                <td colSpan="11" className="py-3 px-4 text-center text-sm sm:text-base">
                  No records found
                </td>
              </tr>
            )}
